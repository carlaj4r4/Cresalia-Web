/**
 * ⏰ Vercel Cron Job: Mensaje Festivo 24 de Diciembre
 * Se ejecuta automáticamente el 24 de diciembre a las 00:00 UTC
 * 
 * Configuración en vercel.json:
 * {
 *   "crons": [{
 *     "path": "/api/cron-mensaje-festivo-24",
 *     "schedule": "0 0 24 12 *"
 *   }]
 * }
 */
...
async function obtenerTodosUsuarios() {
    ...
    // Obtener vendedores/tiendas activas
...
module.exports = async (req, res) => {
    ...
    if (mes !== 11 || dia !== 24) {
        return res.status(200).json({
            success: false,
            message: 'Este cron job solo se ejecuta el 24 de diciembre',
            fechaActual: `${dia}/${mes + 1}/${ahora.getFullYear()}`
        });
    }
    ...
    try {
        ...
        for (const comprador of usuarios.compradores) {
            try {
                ...
                console.error(`❌ Error enviando email a ${comprador.email}:`, error.message);
                emailsError++;
            }
        }
        ...
        for (const vendedor of usuarios.vendedores) {
            try {
                ...
                console.error(`❌ Error enviando email a ${vendedor.email}:`, error.message);
                emailsError++;
            }
        }
        ...
        const resultado = {
            success: true,
            message: 'Mensajes festivos enviados exitosamente',
            fecha: ahora.toISOString(),
            totalUsuarios,
            emailsEnviados,
            emailsError,
            porcentajeExito: ((emailsEnviados / totalUsuarios) * 100).toFixed(2) + '%'
        };

        console.log('✅ Envío masivo completado:', resultado);
