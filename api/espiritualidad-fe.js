/**
 * API Router: Comunidad Espiritualidad y Fe
 * GET /api/espiritualidad-fe?tipo=publicaciones&categoria=...
 * POST /api/espiritualidad-fe?tipo=publicaciones
 */
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (tipo === 'publicaciones') {
            if (req.method === 'GET') {
                let url = `${SUPABASE_URL}/rest/v1/espiritualidad_publicaciones?order=created_at.desc&estado=eq.activa`;
                ...
                const response = await fetch(`${SUPABASE_URL}/rest/v1/espiritualidad_publicaciones`, {
                    method: 'POST',
                    headers: {
                        'apikey': SUPABASE_KEY,
                        'Authorization': `Bearer ${SUPABASE_KEY}`,
        ...
        console.error('Error en API espiritualidad-fe:', error);
