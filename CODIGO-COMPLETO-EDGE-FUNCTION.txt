...
function generarTemplateEmail(alerta: any) {
  ...
  const ubicacion = alerta.ubicacion_texto || 'Ubicación no especificada'
...
<head>
  ...
  <style>
    ...
    .btn { display: inline-block; padding: 12px 24px; background: ${color}; color: white; text-decoration: none; border-radius: 5px; margin: 10px 5px; }
  </style>
</head>
<body>
  <div class="container">
    <div class="header">
      <h1>${icono} ALERTA ${alerta.severidad.toUpperCase()}</h1>
    </div>
    <div class="content">
      <h2>${alerta.titulo}</h2>
      ...
      <div style="margin: 20px 0;">
        ...
        <span class="badge" style="background: #e5e7eb; color: #374151;">
          ${esGlobal ? 'SOLIDARIDAD GLOBAL' : 'ALERTA LOCAL'}
        </span>
      </div>

      <p><strong>Ubicación:</strong> ${ubicacion}</p>
      
      <p style="font-size: 16px; line-height: 1.8;">${alerta.descripcion}</p>

      ${alerta.instrucciones ? `
        <div style="background: #fef3c7; padding: 15px; border-left: 4px solid #f59e0b; margin: 20px 0;">
          <strong>Instrucciones:</strong>
          <p>${alerta.instrucciones}</p>
        </div>
      ` : ''}

      ${!esGlobal && alerta.horas_sin_servicio ? `
        <div style="background: #fee2e2; padding: 15px; border-left: 4px solid #dc2626; margin: 20px 0;">
          <strong>⏰ Tiempo sin servicio:</strong> ${alerta.horas_sin_servicio} horas
        </div>
      ` : ''}
      ...
      ${esGlobal ? `
        <div style="background: #f0fdf4; padding: 15px; border-radius: 8px; margin: 20px 0;">
          <h3 style="margin-top: 0;">🤝 Tu ayuda es valiosa</h3>
          <p>Cualquier contribución, por pequeña que sea, puede marcar la diferencia para las personas afectadas.</p>
        </div>
      ` : ''}
    </div>
