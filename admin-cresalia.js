...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Inicializando Panel de Administración Cresalia...');
...
function actualizarEstadisticasDashboard() {
    ...
    try {
        ...
        const categorias = JSON.parse(localStorage.getItem('categoriasCRESALIA') || '[]');
        const subcategorias = JSON.parse(localStorage.getItem('subcategorias') || '[]');
        
        // Calcular estadísticas reales
        const stats = {
            productos: productos.length,
            categorias: categorias.length + subcategorias.length,
            ventasHoy: 0, // Por ahora 0, se puede implementar después
            clientesNuevos: 0 // Por ahora 0, se puede implementar después
        };
        ...
        if (statCards.length >= 4) {
            statCards[0].querySelector('.stat-info h3').textContent = stats.productos;
            statCards[1].querySelector('.stat-info h3').textContent = stats.categorias;
            statCards[2].querySelector('.stat-info h3').textContent = stats.ventasHoy;
            statCards[3].querySelector('.stat-info h3').textContent = stats.clientesNuevos;
        }
        ...
        const statCards = document.querySelectorAll('.stat-card');
        if (statCards.length >= 4) {
            statCards[0].querySelector('.stat-info h3').textContent = '0';
            statCards[1].querySelector('.stat-info h3').textContent = '0';
            statCards[2].querySelector('.stat-info h3').textContent = '0';
            statCards[3].querySelector('.stat-info h3').textContent = '0';
        }
    }
}
                ...
                <form id="paymentForm" class="config-form">
                        ...
                        <div class="method-content">
                            ...
                            <div class="method-panel" id="alias-panel">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="alias" name="alias" value="${config.alias || ''}" placeholder="Ej: CRESALIA.SAAS" maxlength="6" pattern="[A-Z0-9.]{6}" required>
                                    <small class="form-help">El alias debe tener exactamente 6 caracteres (letras, números y puntos)</small>
                                </div>
                                
                                <div class="form-group">
                                    <label for="cvu">CVU:</label>
                                    <input type="text" id="cvu" name="cvu" value="${config.cvu || ''}" placeholder="0000000000000000000000" maxlength="22" pattern="[0-9]{22}" required>
                                    <small class="form-help">El CVU debe tener exactamente 22 dígitos numéricos</small>
                                </div>
                                
                                <div class="form-group">
                                    <label for="titularAlias">Titular:</label>
                                    <input type="text" id="titularAlias" name="titularAlias" value="${config.titularAlias || ''}" placeholder="Nombre y Apellido del titular" required>
                                </div>
                            </div>
                            ...
                            <div class="method-panel" id="cbu-panel">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="cbuDirecto" name="cbuDirecto" value="${config.cbuDirecto || ''}" placeholder="0000000000000000000000" maxlength="22" pattern="[0-9]{22}" required>
                                    <small class="form-help">El CBU debe tener exactamente 22 dígitos numéricos</small>
                                </div>
                                
                                <div class="form-group">
                                    <label for="bancoCbu">Banco:</label>
                                    <input type="text" id="bancoCbu" name="bancoCbu" value="${config.bancoCbu || ''}" placeholder="Nombre del banco" required>
                                </div>
                                
                                <div class="form-group">
                                    <label for="titularCbu">Titular:</label>
                                    <input type="text" id="titularCbu" name="titularCbu" value="${config.titularCbu || ''}" placeholder="Nombre y Apellido del titular" required>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="payment-settings">
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="mensajeTransferencia" name="mensajeTransferencia" rows="3" placeholder="Mensaje que aparecerá en las transferencias...">${config.mensajeTransferencia || 'Pago CRESALIA - Plataforma SaaS'}</textarea>
                        </div>
                    </div>
            ...
            <div class="config-general-content">
                <form id="contactoForm" class="config-general-form">
                    <div class="form-group">
                        ...
                        <input type="text" id="nombreEmpresa" name="nombreEmpresa" value="${config.nombreEmpresa || 'CRESALIA'}" required>
                    </div>
...
function mostrarFormularioServicio(servicioIndex = null) {
    const servicio = servicioIndex !== null ? 
        JSON.parse(localStorage.getItem('serviciosCRESALIA'))[servicioIndex] : null;
...
function guardarServicio(servicioIndex = null) {
    ...
    const servicios = JSON.parse(localStorage.getItem('serviciosCRESALIA') || '[]');
...
function mostrarFormularioCategoria(categoriaIndex = null) {
    const categoria = categoriaIndex !== null ? 
        JSON.parse(localStorage.getItem('categoriasCRESALIA'))[categoriaIndex] : null;
            ...
            <div class="modal-producto-content">
                <form id="formularioCategoria" class="formulario-producto">
                    ...
                    <div class="form-group">
                        ...
                        <input type="file" id="imagenCategoria" name="imagen" accept="image/*" onchange="previsualizarImagenCategoria(this)">
                        <div id="previewCategoria" class="image-preview" style="display: ${categoria && categoria.imagen ? 'block' : 'none'};">
                            <img id="previewImgCategoria" src="${categoria && categoria.imagen ? categoria.imagen : ''}" alt="Vista previa" style="max-width: 200px; max-height: 200px; border-radius: 8px;">
                        </div>
                        <small class="form-text">Selecciona una imagen desde tu computadora</small>
                    </div>
...
function guardarCategoria(categoriaIndex = null) {
    ...
    if (imagenInput.files && imagenInput.files[0]) {
        ...
        reader.onload = function(e) {
            ...
            guardarCategoriaConImagen(categoriaIndex, formData, imagenBase64);
        };
        
        reader.readAsDataURL(file);
    } else {
        // Si no hay nueva imagen, usar la existente o placeholder
        const categorias = JSON.parse(localStorage.getItem('categoriasCRESALIA') || '[]');
        imagenBase64 = categoriaIndex !== null ? categorias[categoriaIndex]?.imagen || '' : '';
        guardarCategoriaConImagen(categoriaIndex, formData, imagenBase64);
    }
}

function guardarCategoriaConImagen(categoriaIndex, formData, imagenBase64) {
    const categoria = {
        nombre: formData.get('nombre'),
        descripcion: formData.get('descripcion'),
        imagen: imagenBase64,
    ...
    // Guardar en localStorage
    localStorage.setItem('categoriasCRESALIA', JSON.stringify(categorias));
...
function previsualizarImagenCategoria(input) {
    const preview = document.getElementById('previewCategoria');
    const previewImg = document.getElementById('previewImgCategoria');
...
function mostrarFormularioProducto(productoIndex = null) {
    const producto = productoIndex !== null ? 
        JSON.parse(localStorage.getItem('productosCRESALIA'))[productoIndex] : null;
...
function guardarProducto(productoIndex = null) {
    ...
    const productos = JSON.parse(localStorage.getItem('productosCRESALIA') || '[]');
...
function cargarEstadisticas() {
    ...
    mostrarNotificacion('📊 Estadísticas en tiempo real - En desarrollo', 'info');
}
...
console.log('🎯 Panel de Administración CRESALIA - Listo para usar');
console.log('🔐 Contraseña de administrador:', ADMIN_CONFIG.password);
...
function guardarConfiguracionPagos(event) {
    ...
    const camposRequeridos = ['banco', 'titular', 'tipoCuenta', 'numeroCuenta', 'cbu'];
    const camposVacios = camposRequeridos.filter(campo => !config[campo]);
    
    if (camposVacios.length > 0) {
        mostrarNotificacion(`❌ Faltan campos requeridos: ${camposVacios.join(', ')}`, 'error');
        return;
    }
...
function sincronizarConPaginaPrincipal() {
    ...
    const servicios = JSON.parse(localStorage.getItem('serviciosCRESALIA') || '[]');
    if (servicios.length > 0) {
        localStorage.setItem('services', JSON.stringify(servicios));
    ...
    if (Object.keys(configEmpresa).length > 0) {
        localStorage.setItem('companyConfig', JSON.stringify(configEmpresa));
        console.log('✅ Configuración de empresa sincronizada');
    }
    ...
    if (Object.keys(configPagos).length > 0) {
        localStorage.setItem('paymentSettings', JSON.stringify(configPagos));
        console.log('✅ Configuración de pagos sincronizada');
    }
    
    mostrarNotificacion('🔄 Datos sincronizados con la página principal', 'success');
}
...
function exportarDatos() {
    const datosCompletos = {
        productos: JSON.parse(localStorage.getItem('productosCRESALIA') || '[]'),
        servicios: JSON.parse(localStorage.getItem('serviciosCRESALIA') || '[]'),
        categorias: JSON.parse(localStorage.getItem('categoriasCRESALIA') || '[]'),
...
function importarDatos(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            // Validar estructura
            if (datos.productos) localStorage.setItem('productosCRESALIA', JSON.stringify(datos.productos));
            if (datos.servicios) localStorage.setItem('serviciosCRESALIA', JSON.stringify(datos.servicios));
            if (datos.categorias) localStorage.setItem('categoriasCRESALIA', JSON.stringify(datos.categorias));
...
function realizarBackupAutomatico() {
    ...
    const datosCompletos = {
        ...
        tipo: 'automatico',
        version: '1.3',
        descripcion: 'Backup automático del sistema CRESALIA'
    };
...
function restaurarDesdeBackup(index) {
    ...
    if (index >= 0 && index < backups.length) {
        ...
        // Restaurar datos
        if (backup.productos) localStorage.setItem('productosCRESALIA', JSON.stringify(backup.productos));
        if (backup.servicios) localStorage.setItem('serviciosCRESALIA', JSON.stringify(backup.servicios));
        if (backup.categorias) localStorage.setItem('categoriasCRESALIA', JSON.stringify(backup.categorias));
...
function renderizarTickets(tickets) {
    ...
    return tickets.map(ticket => `
        <div class="ticket-item ${ticket.prioridad} ${ticket.estado}" onclick="abrirTicket(${ticket.id})">
            <div class="ticket-header">
                ...
                <div class="ticket-meta">
                    <span class="ticket-prioridad ${ticket.prioridad}">${ticket.prioridad.toUpperCase()}</span>
                    <span class="ticket-estado ${ticket.estado}">${ticket.estado.replace('_', ' ')}</span>
                </div>
            </div>
            ...
            <div class="ticket-details">
                ...
                <div class="ticket-footer">
                    ...
                    <span class="ticket-fecha">
                        <i class="fas fa-calendar"></i> ${new Date(ticket.fecha).toLocaleDateString('es-AR')}
                    </span>
                    <span class="ticket-tiempo">
                        <i class="fas fa-clock"></i> ${calcularTiempoTranscurrido(ticket.fecha)}
                    </span>
                </div>
            </div>
        </div>
...
function renderizarChatSessions(sessions) {
    ...
    return sessions.map(session => `
        <div class="chat-session ${session.estado}" onclick="abrirChat(${session.id})">
            <div class="chat-avatar">
                <i class="fas fa-user"></i>
            </div>
            <div class="chat-info">
                <h4>${session.cliente}</h4>
                <p>${session.ultimoMensaje}</p>
            </div>
            <div class="chat-meta">
                <span class="chat-tiempo">${calcularTiempoTranscurrido(session.ultimaActividad)}</span>
...
function asignarTicket(ticketId) {
    ...
    const agentes = ['Carla (CRESALIA)', 'Soporte Técnico', 'Administración'];
...
function enviarMensajeChat(event, sessionId) {
    ...
    const mensajeHTML = `
        <div class="mensaje agente">
            <div class="mensaje-header">
                <strong>CRESALIA - Soporte</strong>
...
function guardarServicio(servicioIndex = null) {
    ...
    const servicio = {
        id: servicioIndex !== null ? 
            JSON.parse(localStorage.getItem('serviciosCRESALIA') || '[]')[servicioIndex].id : 
            'servicio-' + Date.now(),
        nombre: formData.get('nombre'),
        descripcion: formData.get('descripcion'),
...
// ===== FUNCIONES FALTANTES PARA SUBCATEGORÍAS =====

// Función para editar subcategoría
function editarSubcategoria(index) {
    mostrarFormularioSubcategoria(index);
}
...
function eliminarSubcategoria(index) {
    ...
    const confirmacion = confirm(`¿Estás seguro de que quieres eliminar la subcategoría "${subcategoria.nombre}"?`);
...
// ===== FUNCIONES COMPLETAS PARA SUBCATEGORÍAS =====

// Función para mostrar formulario de subcategoría
function mostrarFormularioSubcategoria(subcategoriaIndex = null) {
    const modal = document.createElement('div');
    modal.className = 'modal-overlay';
    modal.innerHTML = `
        <div class="modal-content">
            <div class="modal-header">
                <h3>${subcategoriaIndex !== null ? 'Editar' : 'Agregar'} Subcategoría</h3>
                <button class="btn-close" onclick="cerrarModalSubcategoria()">&times;</button>
            </div>
            <form id="formularioSubcategoria" class="modal-body">
                <div class="form-group">
                    <label for="nombreSubcategoria">Nombre de la Subcategoría *</label>
                    <input type="text" id="nombreSubcategoria" name="nombre" required 
                           value="${subcategoriaIndex !== null ? JSON.parse(localStorage.getItem('subcategorias') || '[]')[subcategoriaIndex]?.nombre || '' : ''}">
                </div>
                <div class="form-group">
                    <label for="categoriaPadreSubcategoria">Categoría Padre *</label>
                    <select id="categoriaPadreSubcategoria" name="categoriaPadre" required>
                        <option value="">Selecciona una categoría</option>
                        ${JSON.parse(localStorage.getItem('categorias') || '[]').map(cat => 
                            `<option value="${cat.id}" ${subcategoriaIndex !== null && JSON.parse(localStorage.getItem('subcategorias') || '[]')[subcategoriaIndex]?.categoriaPadre === cat.id ? 'selected' : ''}>
                                ${cat.nombre}
                            </option>`
                        ).join('')}
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea id="descripcionSubcategoria" name="descripcion" rows="3">${subcategoriaIndex !== null ? JSON.parse(localStorage.getItem('subcategorias') || '[]')[subcategoriaIndex]?.descripcion || '' : ''}</textarea>
                </div>
                <div class="form-group">
                    <label for="colorSubcategoria">Color</label>
                    <input type="color" id="colorSubcategoria" name="color" 
                           value="${subcategoriaIndex !== null ? JSON.parse(localStorage.getItem('subcategorias') || '[]')[subcategoriaIndex]?.color || '#60A5FA' : '#60A5FA'}">
                </div>
                <div class="form-actions">
                    <button type="button" class="btn btn-secondary" onclick="cerrarModalSubcategoria()">Cancelar</button>
                    <button type="submit" class="btn btn-primary">
                        ${subcategoriaIndex !== null ? 'Actualizar' : 'Crear'} Subcategoría
                    </button>
                </div>
            </form>
        </div>
    ...
    // Configurar evento del formulario
    document.getElementById('formularioSubcategoria').addEventListener('submit', function(e) {
        e.preventDefault();
        guardarSubcategoria(subcategoriaIndex);
    });
...
// Función para cerrar modal de subcategoría
...
// Función para guardar subcategoría
function guardarSubcategoria(subcategoriaIndex = null) {
    const form = document.getElementById('formularioSubcategoria');
    const formData = new FormData(form);
    
    const subcategoria = {
        id: subcategoriaIndex !== null ? JSON.parse(localStorage.getItem('subcategorias') || '[]')[subcategoriaIndex].id : Date.now(),
        nombre: formData.get('nombre'),
        categoriaPadre: parseInt(formData.get('categoriaPadre')),
        descripcion: formData.get('descripcion'),
        color: formData.get('color'),
        fechaCreacion: subcategoriaIndex !== null ? JSON.parse(localStorage.getItem('subcategorias') || '[]')[subcategoriaIndex].fechaCreacion : new Date().toISOString(),
        fechaActualizacion: new Date().toISOString()
    };
    
    const subcategorias = JSON.parse(localStorage.getItem('subcategorias') || '[]');
    
    if (subcategoriaIndex !== null) {
        // Editar subcategoría existente
        subcategorias[subcategoriaIndex] = subcategoria;
        mostrarNotificacion('✅ Subcategoría actualizada correctamente', 'success');
    } else {
        // Crear nueva subcategoría
        subcategorias.push(subcategoria);
        mostrarNotificacion('✅ Subcategoría creada correctamente', 'success');
    }
